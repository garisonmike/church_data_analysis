...
class _FirstLaunchBackupImportScreenState
  ...
  Future<void> _restoreBackup() async {
    ...
    try {
      // Shared restore implementation: validates, parses, and writes
      // atomically (churches → admins → records with FK remapping; orphaned
      // admin references become null; any failure rolls back everything).
      ...
      if (!result.success) {
        ...
        setState(() {
          _state = _RestoreState.error;
          _errorMessage = result.error ?? 'Restore failed. No data was changed.';
        });
        return;
      }
